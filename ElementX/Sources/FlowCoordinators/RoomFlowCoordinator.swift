...
enum RoomFlowCoordinatorAction: Equatable {
    case presentCallScreen(roomProxy: JoinedRoomProxyProtocol, mediaType: ElementCallMediaType)
    ...
    static func == (lhs: RoomFlowCoordinatorAction, rhs: RoomFlowCoordinatorAction) -> Bool {
        switch (lhs, rhs) {
        case (.presentCallScreen(let lhsRoomProxy, let lhsMediaType), .presentCallScreen(let rhsRoomProxy, let rhsMediaType)):
            lhsRoomProxy.id == rhsRoomProxy.id && lhsMediaType == rhsMediaType
...
class RoomFlowCoordinator: FlowCoordinatorProtocol {
    ...
    private func makeRoomScreenCoordinator(presentationAction: PresentationAction?, animated: Bool) -> RoomScreenCoordinator {
        ...
        coordinator.actions
            .sink { [weak self] action in
                ...
                case .presentCallScreen(let mediaType):
                    actionsSubject.send(.presentCallScreen(roomProxy: roomProxy, mediaType: mediaType))
    ...
    private func presentRoomDetails(isRoot: Bool, animated: Bool) async {
        ...
        coordinator.actions.sink { [weak self] action in
            ...
            case .presentCall:
                actionsSubject.send(.presentCallScreen(roomProxy: roomProxy, mediaType: .video))
            case .presentPinnedEventsTimeline:
                stateMachine.tryEvent(.presentPinnedEventsTimeline)
    ...
    private func startChildFlow(for roomID: String, via: [String], entryPoint: RoomFlowCoordinatorEntryPoint) {
        ...
        coordinator.actions.sink { [weak self] action in
            ...
            switch action {
            case .presentCallScreen(let roomProxy, let mediaType):
                actionsSubject.send(.presentCallScreen(roomProxy: roomProxy, mediaType: mediaType))
