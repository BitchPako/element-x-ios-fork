...
enum ChatsTabFlowCoordinatorAction {
    ...
    case showCallScreen(roomProxy: JoinedRoomProxyProtocol, mediaType: ElementCallMediaType)
    ...
    private func startRoomFlow(roomID: String,
        ...
        coordinator.actions.sink { [weak self] action in
            ...
            case .presentCallScreen(let roomProxy, let mediaType):
                actionsSubject.send(.showCallScreen(roomProxy: roomProxy, mediaType: mediaType))
        ...
        coordinator.actionsPublisher
            .sink { [weak self] action in
                ...
                case .presentCallScreen(let roomProxy):
                    actionsSubject.send(.showCallScreen(roomProxy: roomProxy, mediaType: .video))
                case .verifyUser(let userID):
                    actionsSubject.send(.sessionVerification(.userInitiator(userID: userID)))
                case .finished:
